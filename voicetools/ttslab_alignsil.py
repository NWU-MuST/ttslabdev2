...
def transplant_segtime_info(voice, sc_utt, utt):
    ...
    #get silence and closure phones..
    silence_phone = voice.pronun["main"]["phoneset"].features["silence_phone"]
    closure_phone = voice.pronun["main"]["phoneset"].features["closure_phone"]
    ...
    while seg_iter is not None:
        if sc_utt.entries[index][1] == phmap[closure_phone]:
            seg_iter["cl_end"] = float(sc_utt.entries[index][0])
            index += 1
        if phmap[seg_iter["name"]] == sc_utt.entries[index][1]:
            seg_iter["end"] = float(sc_utt.entries[index][0])
            seg_iter = seg_iter.next_item
            index += 1
        elif sc_utt.entries[index][1] == phmap[silence_phone]: #non-obvious logic :-(
            sil_item = seg_iter.prepend_item()
            sil_item["name"] = silence_phone
            sil_item["end"] = float(sc_utt.entries[index][0])
            index += 1
        else:
...
def main():
    ...
    try:
        ...
        print("USAGE: ttslab_alignsil.py [VOICEFILE] [auto | to_textgrid  | from_textgrid]")
